#This code is a python version of halofit. It gives nonlinear correction on matter power spectrum. The power spectrum is interpolated from a linear mps given by CAMB. The formulas are in Appendix of 1208.2701.
...
import matterps, const, growthfactor, camb_interp
...
def solveforr(rd = 0,d = 1): #Solve for k_sigma^(-1) with sigma^2(R) = 1
    global om0,Rnl,neff,C,an,bn,cn,gamman,alphan,betan,mun,nun,f1,f2,f3
    i = 1
    xlogr1 = -5.0
    xlogr2 = 5
    while i == 1:
...
def supparams(z = 0, growthf = 1): #supplementary parameters(A5-A13)
    ...
    an = 10 ** (1.5222 + 2.8553 * neff + 2.3706 * neff ** 2 + 0.9903 * neff ** 3 + 0.225 * neff ** 4 - 0.6038 * C + 0.1749 * omq * (1 + w))
    bn = 10 ** (-0.5642 + 0.5864 * neff + 0.5716 * neff ** 2 - 1.5474 * C + 0.2279 * omq * (1 + w))
    ...
    betan = 2.0379 - 0.7354 * neff + 0.3157 * neff ** 2 + 1.249 * neff ** 3 + 0.3980 * neff ** 4 - 0.1682 * C
    mun = 0
...
def deltaq(k,z = 0): #Two halo terms(A2)
    ...
    deltaq = deltal * ((1 + deltal) ** betan / (1 + alphan * deltal)) * np.exp(- funy(y))
    return deltaq

def deltah(k): #One halo terms(A3)
    global om0,Rnl,neff,C,an,bn,cn,gamman,alphan,betan,mun,nun,f1,f2,f3
    y = k * Rnl
    deltaprime = an * y ** (3 * f1) / (1 + bn * y ** f2 + (cn * f3 * y) ** (3 - gamman))
