...
# NOTE: all spectrum calculated by this code depends on k scaled with Mpc^-1.
# If you need to use h Mpc^-1 as unit, please set yourself.
...
def delta2(k):
    ...
    oml = Lambda / (1 * H0 ** 2)  # omega_Lambda
    omr = 1 - om0 - oml
    # power index, set to 1 for Harrison-Zel'dovich-Peebles
...
# calculate the normalization of the power spectrum
# (often use sigma(8)). Note that k is in the unit of Mpc^-1#
    ...
    if s8 == 0:
        def func(x):
            def j1(z):
                ...
                return j
            return (1 / x) * delta2(x) * (3 * j1(8 / const.cosmo['h'] * x) /
                                          (8 / const.cosmo['h'] * x)) ** 2
