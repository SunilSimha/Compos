#This routine calls camb to calculate transferfunction with a given set of cosmological parameters.
...
def cambbypy(kmax = 10): #Call CAMB by python code.
    ...
    os.system('./camb paramsformps.ini > ' + path + '/scripts/results/callcamb/spectra/cambscreen'+str(ombh2)+','+str(omch2)+','+str(omnuh2)+','+str(omk)+','+str(hubble)+','+str(temp_cmb)+'.txt')
    return
...
def readtransfunction(): 
    ...
    os.path.join(path,'scripts/results/callcamb/spectra')
    os.chdir(path)
    kt = np.loadtxt('test('+str(ombh2)+','+str(omch2)+','+str(omnuh2)+','+str(omk)+','+str(hubble)+','+str(temp_cmb)+')_transfer_out.dat')
    kt = np.transpose(kt)
    return kt
