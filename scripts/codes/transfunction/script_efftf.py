# This script calls transfunction.py to calculate
# transfunction with no oscillation.
...
mp.text(plottextl, plottextd, r'$h = $' + str(const.cosmo['h']) +
        r'$, \Omega_b h^2 = $' + str(const.cosmo['h'] ** 2 *
                                     const.cosmo['omega_b']) +
        r'$, \Omega_c h^2 =$' + str(const.cosmo['h'] ** 2 *
                                    const.cosmo['omega_c']), fontsize=15)
mp.ylabel('T(k)')
mp.xticks([])
